...
def process_files(res_path, out_path):
    ...
    os.makedirs(f"{dataset_dir}/{variant_name}", exist_ok=True)

    output_path_pre = f"{dataset_dir}/{variant_name}/{variant_name}_pre.csv"
    output_path_post = f"{dataset_dir}/{variant_name}/{variant_name}_post.csv"
...
def process_csv_files(output_path_pre, output_path_post):
    output_path = "/".join(output_path_pre.split("/")[0:-1])
