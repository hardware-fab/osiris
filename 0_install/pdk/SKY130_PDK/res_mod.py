...
class ResGenerator(DefaultCanvas):
    def __init__(self, pdk, fin, finDummy):
        ...
        self.m1res = self.addGen( Region('m1res', 
                                         'M1', 
                                         v_grid=CenteredGrid(offset=self.pdk['M2']['Pitch']//2, pitch=self.pdk['M2']['Pitch']),
                                         h_grid=CenteredGrid(offset=self.pdk['M2']['Pitch']//2, pitch=self.pdk['M2']['Pitch']) 
                                        )
                                )

        #self.li1 = self.addGen( Region('li1', 
        #                                 'Lires', 
        #                                 v_grid=CenteredGrid(offset=self.pdk['M2']['Pitch']//2, pitch=self.pdk['M2']['Pitch']),
        #                                 h_grid=CenteredGrid(offset=self.pdk['M2']['Pitch']//2, pitch=self.pdk['M2']['Pitch']) 
        #                                )
        ...
        self.m1res2 = self.addGen( Region('m1res2', 
                                          'M1', 
                                          v_grid=CenteredGrid(offset=self.pdk['Cap']['m1Pitch']//2, pitch=self.pdk['Cap']['m1Pitch']),
                                          h_grid=CenteredGrid(offset=self.pdk['Cap']['m1Pitch']//2, pitch=self.pdk['Cap']['m1Pitch']) 
                                        )
                                 )
        
        self.li1_2 = self.addGen( Region('li1_2', 
                                          'Lires', 
                                          v_grid=CenteredGrid(offset=self.pdk['Cap']['m1Pitch']//2, pitch=self.pdk['Cap']['m1Pitch']),
                                          h_grid=CenteredGrid(offset=self.pdk['Cap']['m1Pitch']//2, pitch=self.pdk['Cap']['m1Pitch']) 
                                        )
        ...
        self.li1 = self.addGen( Wire( 'li1', 'Lires', 'v',
                                     clg=ColoredCenterLineGrid( colors=['c1','c2'], pitch=self.pdk['Cap']['m1Pitch'], width=self.pdk['Cap']['m1Width']),
                                     spg=EnclosureGrid( pitch=self.pdk['M2']['Pitch'], stoppoint=self.pdk['V1']['VencA_L'] +self.pdk['Cap']['m2Width']//2, check=True)))
        ...
        # self.li1_2 = self.addGen( Wire( 'li1_2', 'Lires', 'h',
        #                              clg=ColoredCenterLineGrid( colors=['c1','c2'], pitch=self.pdk['M2']['Pitch'], width=self.pdk['Cap']['m1Width']),
        #                              spg=EnclosureGrid( pitch=self.pdk['Cap']['m1Pitch'], stoppoint=self.pdk['Cap']['m1Width']//2, check=False)))
        ...
        self.v1res = self.addGen( Via( 'v1res', 'V1',
                                        h_clg=self.m2res.clg, v_clg=self.m1res.v_grid,
        ...
        #self.li1 = self.addGen( Region( 'li1', 'Lires', h_grid=self.m1res.clg, v_grid=self.m1res.clg))
    ...
    def _addRes( self, x, y, height, unit_res, draw_boundary=True):
        ...
        res_per_area =12.8
        area = unit_res/res_per_area
        width = round(area/height)

        self.addRegion(self.m1res, None, (1, -9), -1, (7, -1), 17 )
        #self.addRegion(self.li1,   None, (1, -9), -1, (7, -1), 17 )
        #self.addWire( self.li1, None, -2, (21, -7), (7, -1), netType='res')

        for i in range(x_number):
            (k, p) = (2*i, 1) if x_number==2 else (i, 0)
            grid_x = k + x*grid_cell_x_pitch
            # self.addWire( self.m1res, None, grid_x, (grid_y0, -1), (grid_y1, 1))
            # self.addWire( self.li1, None, grid_x, (grid_y0, -1), (grid_y1, 1), netType='res')
            if i < x_number-1:
                grid_yh = ((i+1)%2)*last_y1_track
                # self.addWire( self.m1res2, None, grid_yh, (i, -1), (i+p+1, 1))
                # self.addWire( self.li1_2, None, grid_yh, (i, -1), (i+p+1, 1), netType='res')
#
...
#
        ...
        pin = 'PLUS'
        lower_y_coord = 1
        self.addWire( m2n, 'PLUS', lower_y_coord, (-4, -1), (-2, 1), netType = 'pin')
        self.addVia( self.v1res, None, -5, lower_y_coord)
        pin = 'MINUS'
        #self.addWire( self.m2res, 'MINUS', grid_y, (grid_x1+p, -1), (grid_x1+p+4, 1), netType = 'pin')
        upper_y_coord = 8
        self.addWire( self.m2res, 'MINUS', upper_y_coord, (5, 1), (9, 1), netType = 'pin')
        self.addVia( self.v1res, None, 11, upper_y_coord)
        if draw_boundary:
