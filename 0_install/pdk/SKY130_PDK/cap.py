...
class CapGenerator(DefaultCanvas):
    ...
    def __init__(self, pdk):
        ...
        self.m5_offset = self.enclosure + self.pdk['CapMIMContact']['Enclosure'] + self.pdk['CapMIMContact']['WidthX']//2
        self.m5n = self.addGen(Wire( 'm5n', 'M5', 'h',
    ...
    def addCap( self, length, width):
        ...
        m4n_xwidth = x_length + 2*self.enclosure
        m4n_ywidth = y_length + 2*self.enclosure
        
        m4n = Wire( 'm4n', 'M4', 'v',
                    clg=UncoloredCenterLineGrid( pitch=2*m4n_xwidth, width=m4n_xwidth, offset=m4n_xwidth//2),
                    spg=EnclosureGrid(pitch=y_length, stoppoint=self.enclosure, check=False))
        ...
        mimcap = Wire( 'mim', 'CapMIMLayer', 'v',
                                     clg=UncoloredCenterLineGrid( pitch=2*x_length, width=x_length, offset=x_length//2+self.enclosure),
                                     spg=EnclosureGrid(pitch=y_length, stoppoint=0, check=False))


        x_number = math.ceil(m4n_xwidth/m1_p)
        y_number_m4 = math.ceil((y_length+self.enclosure+0.5*self.pdk['M4']['Width'])/self.pdk['M4']['Pitch'])
        y_number = math.ceil((y_number_m4*self.pdk['M4']['Pitch'])/m2_p)

        logger.debug( f"Number of wires {x_number} {y_number}")

        self.addWire( m4n, 'PLUS', 0, (0, -1), (1, 1))
        self.addWire( m4n_plate, 'MINUS', 1, (y_number_m4-1-1, -1), (y_number_m4, 1))
        self.addWire( mimcap, 'PLUS', 0, (0, -1), (1, 1))
        self.addWire( self.m5n, 'PLUS', 0, (-3, 1), (1, 1)) 
        self.addVia( self.v4_x, 'PLUS', -216, 200)
        self.addVia( self.v3_x, 'PLUS', -216, 200)
        ...
        self.addWire( self.m3, 'PLUS', -1, (-1, -1), (gridx2, 1), netType = 'pin')
 
        self.addRegion( self.boundary, 'Boundary', -2-self.HALO_MULT, -6-self.HALO_MULT,
                        x_number+1+self.HALO_MULT,
                        y_number+3+self.HALO_MULT)
