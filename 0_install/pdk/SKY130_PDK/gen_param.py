...
def gen_param(subckt, primitives, pdk_dir):
    ...
    if generator_name == 'CAP':
        ...
        assert float(values["VALUE"]) or float(values["R"]), f"unidentified size {values['VALUE']} for {ele.name}"
        if "R" in values:
        ...
        for key in mvalues:
            ...
            #assert int(float(mvalues[key]["W"])*1E+9) % design_config["Fin_pitch"] == 0, \
        ...
        if isinstance(size, int):
            for key in mvalues:
                assert int(mvalues[device_name]["NFIN"]) == int(mvalues[key]["NFIN"]), f"W should be same for all devices in {block_name} {mvalues}"
